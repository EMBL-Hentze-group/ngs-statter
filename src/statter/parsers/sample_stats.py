...
class SampleStats:
    ...
    def _add_trimming_stats(self) -> None:
        ...
        self._sample_stats["Raw reads"] = first_trim["summary"]["before_filtering"][
            "total_reads"
        ]
        for comb in (first_trim["filtering_result"], second_trim["filtering_result"]):
            for k, v in comb.items():
                ...
                try:
                    ...
                    self._sample_stats[stat_name] = v
        self._sample_stats["Trimmed reads"] = second_trim["summary"]["after_filtering"][
            "total_reads"
        ]
    ...
    def _add_rRNA_stats(self) -> None:
        ...
        if self._sample_stats["Trimmed reads"] != rRNA_mapped + rRNA_free:
            raise RuntimeError(
                "The number of trimmed reads does not match the number of mapped rRNA + unmapped rRNA reads!"
            )
    ...
    def _add_alignment_stats(self) -> None:
        ...
        if self.check_rRNA and (
            align_stats["Input reads"] != self._sample_stats["rRNA free"]
        ):
            raise RuntimeError(
                "The number of rRNA free reads does not match the number of genome mapped reads!"
            )
...
def _gather_sample_stats(
    stats_dir: Union[str, Path],
    out: str,
    suffix="sample_stats.json",
) -> None:
    """gather_sample_stats
    ...
    if len(samples) == 0:
        raise RuntimeError(
            f"Cannot find sample stats file with suffix {suffix} in directory {str(stats_dir)}"
        )
    stats, names = [], []
    for s in samples:
